...
p.setup = function(){
    ...
    boxes.push(new PhysicsBlock(0, 0, 200, 100, colors[Math.floor(Math.random()*colors.length)], p));
    boxes.push(new PhysicsBlock2(0, 0, 150, 60, colors[Math.floor(Math.random()*colors.length)], p));
...
let upArr = document.getElementById("upArr");
let downArr = document.getElementById("downArr");
let rightArr = document.getElementById("rightArr");

// leftArr.addEventListener("mouseover", function(gravityX, gravityY){
//     // console.log("test");
//     console.log("hi")
// });

let mybuttons = [];
mybuttons = document.getElementsByClassName("button");
// console.log(mybuttons[0].dataset.gravityx);

for (i=0; i<4; i++) {
    // console.log(mybuttons[i])
    let gravityX = mybuttons[i].dataset.gravityx;
    let gravityY = mybuttons[i].dataset.gravityy;

    mybuttons[i].addEventListener("click", function(e){
        // console.log(gravityX) ;
    setGravity(gravityX, gravityY, e);
    
    });
    // console.log("test");
}

function setGravity(x, y, e){
    // console.log(x, y)
    world.gravity.y = y;
    world.gravity.x = x;
    // console.log(e);
...
//   var body = Matter.Bodies.circle(0, 0, 100, {
//     isStatic: true,
//     plugin: {
//       attractors: [
//         function(bodyA, bodyB) {
